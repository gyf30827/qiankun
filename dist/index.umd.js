(function (global, factory) {
	...
	function createElement(appContent, strictStyleIsolation, scopedCSS, appInstanceId) {
	  ...
	  var appElement = containerElement.firstChild; // 通过 shadow dom 的方式进行隔离
	      ...
	      while (1) {
	        switch (_context17.prev = _context17.next) {
	          case 0:
	            ...
	            entry = app.entry, appName = app.name; // 记录并获取 app实例id  没执行一次 genAppInstanceIdByName id 加 1
	          ...
	          case 18:
	            // 将获取到的html内容放置到默认的元素下
	            appContent = getDefaultTplWrapper(appInstanceId)(template); // 是否通过 dom shadow 的方式隔离子应用
	            ...
	            } // 是否给css 加前缀 通过 sandbox 这个 experimentalStyleIsolation 属性配置
	...
	var microApps = []; // 前端的全局配置，在 start 时传入

	var frameworkConfiguration = {};
	var started = false;
	var defaultUrlRerouteOnly = true; // 异步加载器（Promise）

	var frameworkStartedDefer = new Deferred(); // 如果开启了沙箱模式，但是浏览器不支持 proxy 进行降级处理
	        ...
	        var _app = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee3() {
	          ...
	          return regenerator.wrap(function _callee3$(_context3) {
	            while (1) {
	              switch (_context3.prev = _context3.next) {
	                case 0:
	                  loader(true); // 在start 中会将此 promise resolve 掉，才能开始子应用的加载
	...
	function start() {
	  ...
	  var _frameworkConfigurati2 = frameworkConfiguration,
	      ...
	      importEntryOpts = _objectWithoutProperties(_frameworkConfigurati2, _excluded3); // 预加载
	  ...
	  started = true; // 将promise resolve，开始
