 ...
 * If the legacy render function is provide, used as it, otherwise
 * we will insert the app element to target container by qiankun
 * @param appInstanceId
 * @param appContent
 * @param legacyRender
 */
...
export async function loadApp<T extends ObjectType>(
  ...
  // 记录并获取 app实例id  没执行一次 genAppInstanceIdByName id 加 1
  ...
  // 合并声明周期，自定义 & qiankun引擎自带的声明周期 & 运行时声明周期（主要是为了 微应用动态载入脚本样式图片等）
  const {
    beforeUnmount = [],
    afterUnmount = [],
    afterMount = [],
